...
**(d) 하드 앵커 — 실제 피드백 있는 쿼리만 (실데이터 빈약 → 약한 보조항)**
```
L_hard = α · BCE(predictor_score, feedback_label)
```
> 실제 feedback 라벨이 **거의 없으므로** `L_hard`는 v1에서 **작은 α의 보조 손실**로만 둔다. feedback이 부족하면 **클릭/저장 proxy 신호로 대체**하고, 그마저 없으면 0(=teacher 증류만)으로 둔다. 학습이 이 항에 의존하지 않도록 설계한다.
...
- **하드 네거티브**: Memgraph theme/mood/trope("동일 장르·다른 무드"), books 제목정규화(세트책/개정판) → teacher 입력에 섞어 난이도↑.
  - **소싱 seam**: `teacher.hard_negatives.HardNegativeSource` (Protocol). 오프라인/테스트는 `StaticHardNegativeSource`(`--hard-negatives <json>`), 운영은 `MemgraphHardNegativeSource`(cypher executor 주입; 위 두 전략을 쿼리). executor 미주입 시 조용히 빈 결과 대신 에러.
  - **주입 지점**: `collect_snapshot(..., payload_transform=inject_hard_negatives)` — fetch 직후·save 직전에 후보 풀에 섞고 `hard_negative` 마커를 단다. 따라서 teacher가 본 풀 = 스냅샷에 저장된 풀(재현성 앵커가 증강된 풀 기준). adapter가 마커를 `TopaBookCandidate.is_hard_negative`로 파싱.
  - **teacher 신호 분리**: 프롬프트엔 "풀에 distractor가 섞여 있으니 표면 유사성에 속지 말고 낮게 채점"이라고만 알리고 **어떤 후보가 hard negative인지는 표시하지 않는다** → teacher soft score는 정직하게 유지. hard negative는 구성상 **known hard_label=0**으로 `L_hard` 앵커에 별도 공급(soft KD와 hard anchor 이중 신호).
  - **학습 연결**: `train_neural.load_batches`가 `hard_label_map(response)`로 마커→`hard_labels`를 복원해 `build_training_batch(..., hard_labels=...)`에 전달 → `_hard_anchor`(weight=α) 손실이 실제 작동. (이전엔 손실 코드만 있고 데이터가 안 흘러 항상 0이었음.)
